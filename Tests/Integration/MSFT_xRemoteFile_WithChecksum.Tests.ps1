...
try
{
    ...
    $ConfigFile = $PSCommandPath -replace 'Tests.ps1','config.ps1'
    . $ConfigFile

    # Make sure the file to download doesn't exist
    Remove-Item -Path $TestDestinationPath -Force -ErrorAction SilentlyContinue

    Describe "$($script:DSCResourceName)_WithChecksum_Integration" {
        #region DEFAULT TESTS
        It 'Should compile without throwing' {
            {
                & "$($script:DSCResourceName)_WithChecksum_Config" -OutputPath $TestDrive
        ...
        It 'Should have set the resource and all the parameters should match' {
            ...
            $Result.Checksum        | Should -Be $TestChecksum
        }
